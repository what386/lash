...
PROJECTS=("lash:./src/Lash.Cli/Lash.Cli.csproj" "lashc:./src/Lash.Compiler/Lash.Compiler.csproj" "lashfmt:./src/Lash.Formatter/Lash.Formatter.csproj")
...
find_published_executable() {
    ...
    if [ $(is_windows_rid "${rid}") -ne 0 ]; then
        ...
        if [ -f "$expected" ]; then src_file="$expected"; else src_file="$(find "$temp_dir" -maxdepth 1 -type f -name '*.exe' ! -name 'createdump.exe' -print -quit)"; fi
        echo ${src_file}
        return 0
    fi
    local expected="${temp_dir}/${project_stem}"
    if [ -f "$expected" ]; then src_file="$expected"; else src_file="$(find "$temp_dir" -maxdepth 1 -type f -perm -u+x -print -quit)"; fi
    echo ${src_file}
    return 0
}
build_tool_for_platform() {
    ...
    if [ -z "$src_file" ] || [ ! -f "$src_file" ]; then echo -e "${RED}✗ Build completed but executable not found for ${tool_name}${NC}"; rm -rf "$temp_dir"; return 1; fi
    local extension=""
    if [ $(is_windows_rid "${rid}") -ne 0 ]; then
        extension=".exe"
    fi
...
if [ -d "$OUTPUT_DIR" ]; then echo -e "${YELLOW}Cleaning output directory...${NC}"; rm -rf "$OUTPUT_DIR"; fi
mkdir -p "$OUTPUT_DIR"
