...
readonly OUTPUT_DIR="./target/release"
...
is_known_tool() {
    ...
    for entry in "${PROJECTS[@]}"; do
        local -r known=$(printf '%s' $entry | cut -d: -f1)
...
get_output_dirname() {
    ...
    echo "Lash-${platform}_${arch}-${bits}"
    return 0
}
find_published_executable() {
    ...
    if [ $(is_windows_rid "${rid}") -ne 0 ]; then
        local -r expected="${temp_dir}/${project_stem}.exe"
    ...
    local -r expected="${temp_dir}/${project_stem}"
    if [ $(file_exists "${expected}") -ne 0 ]; then
        src_file=${expected}
    else
...
build_tool_for_platform() {
    ...
    local -r temp_dir="./target/release/temp_${tool_name}_${rid}"
    ...
    local -r project_stem=$(basename "$project_path" .csproj)
    local -r src_file=$(find_published_executable "${temp_dir}" "${rid}" "${project_stem}")
    ...
    local -r dest_file="${bundle_dir}/${tool_name}${extension}"
    mv "$src_file" "$dest_file"
    rm -rf "$temp_dir"
    if ! [ $(is_windows_rid "${rid}") -ne 0 ]; then
        chmod +x "$dest_file"
    fi
    local -r size=$(du -h "$dest_file" | cut -f1)
...
build_platform() {
    ...
    local -r bundle_dir="./target/release/$(get_output_dirname "${rid}")"
    local publish_jobs=()
    echo ""
    ...
    mkdir -p "$bundle_dir"
    for entry in "${PROJECTS[@]}"; do
        local -r tool_name=$(printf '%s' $entry | cut -d: -f1)
        local -r project_path=$(printf '%s' $entry | cut -d: -f2-)
        (
    ...
    for job in "${publish_jobs[@]}"; do
        local -r pid=$(printf '%s' $job | cut -d: -f1)
        local -r tool_name=$(printf '%s' $job | cut -d: -f2-)
        wait "${pid}"
        local wait_status=$?
    ...
    local -r bundle_size=$(du -sh "$bundle_dir" | cut -f1)
...
if ((ENABLE_TRIMMING != 0)); then
    if [[ ${#NO_TRIM_TOOLS[@]} == 0 ]]; then
        ...
        readonly disabled=$(list_to_csv "${NO_TRIM_TOOLS[@]}")
...
if [ $(dir_exists "./target/release") -ne 0 ]; then
...
for rid in "${BUILD_PLATFORMS[@]}"; do
    readonly description=$(get_platform_description "${rid}")
