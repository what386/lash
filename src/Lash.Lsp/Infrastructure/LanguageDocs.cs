...
internal sealed class LanguageDocs
{
    private static readonly IReadOnlyDictionary<string, string> Docs = new Dictionary<string, string>(StringComparer.Ordinal)
    {
        ...
        ["end"] = "`end` closes block constructs (`if`, `fn`, `for`, `select`, `while`, `until`, `switch`, `enum`, `subshell`, `coproc`) and preprocessor blocks (`@if`, `@raw`).",
        ["fn"] = "`fn` declares a function. Parameters may include defaults.",
        ["for"] = "`for` iterates arrays/ranges/globs: `for x in expr ... end`.",
        ["select"] = "`select` lowers to Bash `select` and supports arrays, ranges, or globs as option sources.",
        ["while"] = "`while` repeats while condition is truthy.",
        ["until"] = "`until` repeats until condition becomes truthy (inverse of `while`).",
        ["switch"] = "`switch` matches a value against one or more `case` clauses and lowers to Bash `case`.",
        ["case"] = "`case` declares a switch branch: `case expr:` or `case expr1, expr2:`. `_` is the wildcard form.",
        ["var"] = "`var` declares a mutable variable.",
        ["let"] = "`let` declares a compile-time immutable variable (no runtime `readonly` emission).",
        ["readonly"] = "`readonly` declares a runtime shell-immutable variable. It requires an initializer and is not allowed inside repeated loop contexts.",
        ...
        ["coproc"] = "`coproc ... end` lowers to a Bash coprocess block. Use `into` to capture `${COPROC_PID}`.",
        ...
        ["break"] = "`break` exits the current loop. `break N` exits `N` nested loops.",
        ["continue"] = "`continue` skips to the next loop iteration. `continue N` continues `N` nested loops upward.",
        ...
        ["into"] = "`into` captures values from `subshell`, `wait`, and `@import ... into`. Bare `into name` assigns existing variables or creates a new immutable `let` when missing.",
        ["jobs"] = "`jobs` in `wait jobs` waits for tracked background subshells.",
        ["sh"] = "`sh expr` emits shell command payload as a statement. `$(...)` captures shell output as an expression.",
        ["test"] = "`test expr` emits a Bash test command (`[[ ... ]]`). Inside capture, `$(test ...)` returns numeric truth (`1` or `0`).",
        ["trap"] = "`trap SIGNAL into fn()` registers a function trap handler. `trap SIGNAL \"cmd\"` registers raw shell trap code.",
        ...
        ["=~"] = "Regex-match operator. Supported in condition positions and lowers to Bash `[[ lhs =~ rhs ]]`.",
        ["+="] = "Compound assignment. Appends arrays when array-typed, or performs numeric add-assignment when number-typed.",
        ["-="] = "Numeric subtract-assignment (lowers to Bash arithmetic).",
        ["*="] = "Numeric multiply-assignment (lowers to Bash arithmetic).",
        ["/="] = "Numeric divide-assignment (lowers to Bash arithmetic).",
        ["%="] = "Numeric modulo-assignment (lowers to Bash arithmetic).",
        ["++"] = "Numeric increment statement (postfix).",
        ["--"] = "Numeric decrement statement (postfix).",
        [">"] = "Redirect stdout to file (truncate).",
        [">>"] = "Redirect stdout to file (append).",
        ["<<"] = "Stdin string redirection. Non-multiline values lower to Bash here-strings; multiline literals lower to heredoc.",
        ["<<-"] = "Tab-stripping stdin redirection for multiline literals (lowers to Bash `<<-`).",
