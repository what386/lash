...
public partial class BashGenerator
{
    ...
    private static string RenderInterpolatedShellPayload(string template)
    {
        ...
        while (cursor < template.Length)
        {
            ...
            if (openBrace < 0)
            {
                builder.Append(UnescapeShellPayloadText(template[cursor..]));
                break;
            }

            builder.Append(UnescapeShellPayloadText(template[cursor..openBrace]));

            var closeBrace = FindNextUnescaped(template, '}', openBrace + 1);
            if (closeBrace < 0)
            {
                builder.Append(UnescapeShellPayloadText(template[openBrace..]));
                break;
            }
            ...
            if (TryGetIdentifierPath(placeholder, out var path))
                ...
                builder.Append(UnescapeShellPayloadText(template[openBrace..(closeBrace + 1)]));
